    ...
    private const string PLATFORM_DLL = "__Internal";
        ...
        private void Open()
        {
            ...
            if (m_nativeContext != IntPtr.Zero && OpenStream(m_nativeContext, path, (int)offset, (int)length, powerOf2Textures, m_scanDuration, maxSkipFrames))
            {
                ...
                UnityEngine.UI.Image spr = gameObject.GetComponent<UnityEngine.UI.Image>();
                if (spr != null) spr.material = CopyMaterial;
                else gameObject.GetComponent<Renderer>().sharedMaterial = CopyMaterial;
